...
(() => {
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    const impBtn = document.getElementById("impossible");
    const ratingBtns = { easy: easyBtn, hard: hardBtn, impossible: impBtn };
    let flashcards = [];
    let currentIndex = 0;
    ...
    revealBtn.addEventListener("click", () => {
      const card = flashcards[currentIndex];
      flashcardDisplay.textContent = card.answer;
      revealBtn.style.display = "none";
      videoCam.style.display = "block";
      camera.start();
    });
    async function recordAndNext(level) {
      ...
      if (currentIndex < flashcards.length) {
        ...
        flashcardDisplay.textContent = "\u{1F389} Quiz complete!";
      }
    }
    easyBtn.addEventListener("click", () => recordAndNext("easy"));
    hardBtn.addEventListener("click", () => recordAndNext("hard"));
    impBtn.addEventListener("click", () => recordAndNext("impossible"));
    const hands = new import_hands.Hands({
      locateFile: (file) => chrome.runtime.getURL(`libs/mediapipe/${file}`)
    });
    hands.setOptions({
      selfieMode: true,
      maxNumHands: 1,
    ...
    const camera = new import_camera_utils.Camera(videoCam, {
      onFrame: async () => {
    ...
    function onResults(results) {
      ...
      const tToI = distance([tips.thumb.x, tips.thumb.y], [tips.index.x, tips.index.y]);
      const iToM = distance([tips.index.x, tips.index.y], [tips.middle.x, tips.middle.y]);
      if (tToI < 0.05) {
        return void recordAndNext("hard");
      }
      if (tToI > 0.2 && distance([tips.middle.x, tips.middle.y], [tips.ring.x, tips.ring.y]) > 0.2 && distance([tips.ring.x, tips.ring.y], [tips.pinky.x, tips.pinky.y]) > 0.2) {
        return void recordAndNext("impossible");
      }
      const wrist = lm[0];
      const foldedFingers = tips.index.y > wrist.y && tips.middle.y > wrist.y && tips.ring.y > wrist.y && tips.pinky.y > wrist.y;
      if (tips.thumb.y < wrist.y && foldedFingers) {
        return void recordAndNext("easy");
      }
    }
  });
})();
